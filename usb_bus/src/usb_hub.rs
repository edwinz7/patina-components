...
use patina::{
    component::service::uefi_services::{
        event::{Event, EventServicesExt},
        timer_event::{TimerEventServicesExt, TimerType},
        tpl::{PreviousTpl, Tpl as ServiceTpl},
    },
    error::EfiError,
};
...
use crate::usb_bus_defs::{
    UsbDevice, UsbHubApi, UsbHubServices, UsbInterface,
...
pub static USB_HUB_API: UsbHubApi = UsbHubApi {
    init: usb_hub_init,
    get_port_status: usb_hub_get_port_status,
    clear_port_change: usb_hub_clear_port_change,
    set_port_feature: usb_hub_set_port_feature,
    clear_port_feature: usb_hub_clear_port_feature,
    reset_port: usb_hub_reset_port,
    release: usb_hub_release,
};

pub static USB_ROOT_HUB_API: UsbHubApi = UsbHubApi {
    init: usb_root_hub_init,
    get_port_status: usb_root_hub_get_port_status,
    clear_port_change: usb_root_hub_clear_port_change,
    set_port_feature: usb_root_hub_set_port_feature,
    clear_port_feature: usb_root_hub_clear_port_feature,
    reset_port: usb_root_hub_reset_port,
    release: usb_root_hub_release,
};
...
pub unsafe fn usb_hub_ctrl_get_hub_status(hub: &mut UsbDevice, state: &mut UsbPortStatus) -> efi::Status {
    unsafe { usb_ctrl_request(hub, UsbDataDirection::In, USB_REQ_TYPE_CLASS, USB_HUB_TARGET_HUB, USB_HUB_REQ_GET_STATUS as usize, 0, 0, (state as *mut UsbPortStatus).cast(), mem::size_of::<UsbPortStatus>()) }
}
...
pub unsafe fn usb_hub_read_desc(hub: &mut UsbDevice) -> core::result::Result<Vec<u8>, efi::Status> {
...
pub unsafe fn usb_hub_ack_hub_status(hub: &mut UsbDevice) -> efi::Status {
    let mut state = UsbPortStatus { port_status: 0, port_change_status: 0 };
    let status = unsafe { usb_hub_ctrl_get_hub_status(hub, &mut state) };
    if status.is_error() {
        return status;
    }
    if state.port_change_status & (USB_HUB_STAT_C_LOCAL_POWER as u16) != 0 {
        let _ = unsafe { usb_hub_ctrl_clear_hub_feature(hub, USB_HUB_C_HUB_LOCAL_POWER) };
    }
    if state.port_change_status & (USB_HUB_STAT_C_OVER_CURRENT as u16) != 0 {
...
pub unsafe extern "efiapi" fn usb_on_hub_interrupt(
    ...
    let Some(interface) = (unsafe { context.cast::<UsbInterface>().as_mut() }) else {
        return efi::Status::INVALID_PARAMETER;
    };
    let Some(events) = (unsafe { interface.device.as_ref() })
        .and_then(|device| unsafe { device.bus.as_ref() })
        .map(|bus| bus.hub_services.events)
    else {
        return efi::Status::INVALID_PARAMETER;
    };
    ...
    if events.signal_event(event).is_err() {
        return efi::Status::INVALID_PARAMETER;
    }
...
pub unsafe fn usb_hub_init(
    ...
    services: &UsbHubServices,
) -> efi::Status {
    let events = services.events;
    let timing = services.timing;
    interface.is_hub = false.into();
    interface.hub_api = None;
    ...
    let descriptor = match unsafe { usb_hub_read_desc(device) } {
    ...
    interface.hub_api = Some(&USB_HUB_API);
    ...
    let interrupt_context = interface as *mut UsbInterface as *mut c_void;
    let status = unsafe {
        (interface.usb_io.async_interrupt_transfer)(
            &mut interface.usb_io,
...
pub unsafe fn usb_hub_clear_port_change(interface: &mut UsbInterface, port: u8) {
    ...
    if unsafe { usb_hub_get_port_status(interface, port, &mut status) }.is_error() {
        return;
    }
...
pub fn usb_hub_release(interface: &mut UsbInterface, services: &UsbHubServices) -> efi::Status {
    let events = services.events;
    ...
    if !interface.hub_notify.is_null() {
        match Event::from_raw(interface.hub_notify) {
            Some(event) => {
                if events.close_event(event).is_err() && !result.is_error() {
                    result = efi::Status::INVALID_PARAMETER;
                }
            }
            None if !result.is_error() => result = efi::Status::INVALID_PARAMETER,
            None => {}
        }
    }
...
pub unsafe fn usb_root_hub_init(
    ...
    let previous_tpl = tpl.raise_tpl(ServiceTpl::HighLevel);
    tpl.restore_tpl(PreviousTpl::from_raw(efi::TPL_APPLICATION));

    while interface.poll_count < USB_ENUM_POLL_MINIMUM_ATTEMPTS {
        core::hint::spin_loop();
    }

    if previous_tpl.as_raw() > efi::TPL_APPLICATION {
        let previous_level = match previous_tpl.as_raw() {
            efi::TPL_CALLBACK => ServiceTpl::Callback,
            efi::TPL_NOTIFY => ServiceTpl::Notify,
            efi::TPL_HIGH_LEVEL => ServiceTpl::HighLevel,
            _ => ServiceTpl::Application,
        };
        let _ = tpl.raise_tpl(previous_level);
    }
...
pub unsafe fn usb_root_hub_clear_port_change(interface: &mut UsbInterface, port: u8) {
    let mut status = UsbPortStatus { port_status: 0, port_change_status: 0 };
    if unsafe { usb_root_hub_get_port_status(interface, port, &mut status) }.is_error() {
        return;
    }
    for (changed_bit, feature) in [
        (USB_PORT_STAT_C_CONNECTION, UsbPortFeature::ConnectChange),
...
pub fn usb_root_hub_release(
    ...
    if !interface.hub_notify.is_null() {
        match Event::from_raw(interface.hub_notify) {
            Some(event) => {
                if let Err(error) = timers.set_timer(event, TimerType::Cancel) {
                    result = EfiError::from(error).into();
                }
                if let Err(error) = events.close_event(event) {
                    if !result.is_error() {
                        result = EfiError::from(error).into();
                    }
                }
            }
            None => result = efi::Status::INVALID_PARAMETER,
        }
    }
    interface.is_hub = false.into();
    interface.hub_api = None;
